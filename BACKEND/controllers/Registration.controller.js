...
const registerTeam = asyncHandler(async (req, res) => {
  const registration = await RegistrationService.registerTeam(req.hackathon, req.team, req.user._id);
...
const getHackathonRegistrations = asyncHandler(async (req, res) => {
  const registrations = await RegistrationService.getHackathonRegistrations(req.hackathon, req.query);
...
const approveRegistration = asyncHandler(async (req, res) => {
  const registration = await RegistrationService.approveRegistration(req.registration, req.user._id);
...
const rejectRegistration = asyncHandler(async (req, res) => {
  const registration = await RegistrationService.rejectRegistration(req.registration, req.user._id);
