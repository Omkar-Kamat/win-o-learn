...
hackathonScopedSubmissionRoutes.post('/:hackathonId/submissions', VerifyToken, AuthorizeRoles(ROLES.PARTICIPANT), validateHackathonIdParam, LoadHackathon, LoadRegistrationForSubmission({
  requireLeader: true
}), validateCreateSubmission, SubmissionController.createSubmission);
hackathonScopedSubmissionRoutes.get('/:hackathonId/submissions/mine', VerifyToken, AuthorizeRoles(ROLES.PARTICIPANT), validateHackathonIdParam, LoadHackathon, LoadRegistrationForSubmission({
  requireMember: true
...
submissionRoutes.put('/:id', VerifyToken, AuthorizeRoles(ROLES.PARTICIPANT), validateSubmissionIdParam, LoadSubmission({
  requireLeader: true
}), validateUpdateSubmission, SubmissionController.updateSubmission);
submissionRoutes.put('/:id/files', VerifyToken, AuthorizeRoles(ROLES.PARTICIPANT), validateSubmissionIdParam, LoadSubmission({
  requireLeader: true
}), validateUpdateSubmissionFiles, SubmissionController.updateSubmissionFiles);
submissionRoutes.patch('/:id/status', VerifyToken, AuthorizeRoles(ROLES.ORGANIZER), validateSubmissionIdParam, LoadSubmission({
  requireOrganizer: true
}), validateSubmissionStatus, SubmissionController.updateSubmissionStatus);
