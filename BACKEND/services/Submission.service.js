...
const createSubmission = async (hackathon, registration, body) => {
    ...
    return SubmissionRepository.create({ registration: registration._id, ...body });
};
...
const getHackathonSubmissions = async (hackathonId) =>
    SubmissionRepository.findAllByHackathon(hackathonId);
...
const updateSubmissionStatus = async (submission, status) =>
    SubmissionRepository.updateById(submission._id, { status: status });
export default {
