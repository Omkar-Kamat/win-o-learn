 ...
 * Description: Implementation of Dashboard.service.js
 */
...
class DashboardService {
    ...
    // Retrieves organizer dashboard statistics
async getOrganizerDashboard(user) {
    const hackathons = await Hackathon.find({
        organizer: user._id,
    }).select('_id resultsPublished');
    ...
    return {
        ...
        totalSubmissions:
            totalSubmissions.length > 0
                ...
                : 0,
        winnersAnnounced: hackathons.filter(
            (hackathon) => hackathon.resultsPublished
        ).length,
    };
}

    // Retrieves participant dashboard statistics
async getParticipantDashboard(user) {
    ...
    return {
        ...
        submissions: submissions.map((submission) => ({
            ...
            hackathon:
                submission.registration.hackathon.title,
        })),

        resultsPublished: registrations.filter(
            (registration) =>
                registration.hackathon.resultsPublished
        ).length,
    };
}

    // Retrieves judge dashboard statistics
async getJudgeDashboard(user) {
    const assignments = await JudgeAssignment.find({
        judge: user._id,
    });

    const hackathonIds = assignments.map(
        (assignment) => assignment.hackathon
    );
    ...
    const pendingReviews = assignedSubmissions.filter(
        (submission) =>
            !reviewedSubmissionIds.has(submission._id.toString())
    ).length;
