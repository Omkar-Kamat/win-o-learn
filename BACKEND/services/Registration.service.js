...
const registerTeam = async (hackathon, team, userId) => {
    ...
    return await RegistrationRepository.create({ hackathon: hackathon._id, team: team._id });
};
...
const getRegistrationStatus = async (hackathon, team) => {
    ...
    return { registered: true, status: registration.status };
};
const getHackathonRegistrations = async (hackathon, filters = {}) => {
    const page = Number(filters.page) || 1;
    const limit = Number(filters.limit) || 20;
    const { registrations: registrations, total: total } =
